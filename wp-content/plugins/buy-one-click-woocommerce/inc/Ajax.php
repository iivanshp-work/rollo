...
class BuyJavaScript {
    ...
    protected static function checkRequireField($form) {
        ...
        if (!empty($params['email_verifi']) && empty($form['user_email'])) {
            throw new Exception(pll__("<strong>Email</strong> - обов'язкове поле.", 'coderun-oneclickwoo'), 200);
        }
        
        if (!empty($params['fio_verifi']) && empty($form['user_name'])) {
            throw new Exception(pll__("<strong>Ім’я та прізвище</strong> - обов'язкове поле.", 'coderun-oneclickwoo'), 200);
        }
        
        if (!empty($params['fon_verifi']) && empty($form['user_phone'])) {
            throw new Exception(pll__("<strong>Телефон</strong> - обов'язкове поле.", 'coderun-oneclickwoo'), 200);
        }
        
        if (!empty($params['dopik_verifi']) && empty($form['user_cooment'])) {
            throw new Exception(pll__("<strong>Опис</strong> - обов'язкове поле.", 'coderun-oneclickwoo'), 200);
        }
        
        if (!empty($params['conset_personal_data_enabled']) && empty(boolval($form['check_conset_personal_data']))) {
            throw new Exception(pll__('Вам потрібно дати згоду', 'coderun-oneclickwoo'), 200);
        }
        //Файлы
        ...
        if (!empty($params['upload_input_file_chek']) && !empty($params['upload_input_file_verifi']) && empty($files)) {
            throw new Exception(pll__('Attach file', 'coderun-oneclickwoo'), 200);
        }
    }
    ...
    public function ajaxBuyButtonForm() {
        ...
        try {
            ...
            wp_send_json_error(array('message' => "<ul><li>" . $ex->getMessage() . "</li></ul>"));
        }
    ...
    public function ajaxRemoveOrderId() {
        ...
        if(!empty($_POST['text'])) {
            ...
            $infotovar_old = get_option('buyzakaz');
            if(!empty($infotovar_old[$plugin_id]['woo_order_id'])) {

                if(!Help::getInstance()->isset_woo_order($infotovar_old[$plugin_id]['woo_order_id'])) {
                    wp_send_json_error();
                }
                
                $order = new \WC_Order($infotovar_old[$plugin_id]['woo_order_id']);
                if($order->delete()){
                    wp_send_json_success();
                }
            }
    ...
    public static function ajaxgetViewForm() {
        ...
        $cartinfo = self::getProductInfo($product_id);
        $cartinfo['custom'] = 0;
    ...
    public static function getProductInfo($product_id = null) {
        $result = array();
        ...
        if (function_exists('calculate_product_price')) {
            $amount = calculate_product_price($product->get_id());
        } else {
