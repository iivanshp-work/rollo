...
akeeba.Update.extractErrorCallback=function(a){var b=document.getElementById("extractProgressInfo"),c=document.getElementById("extractProgressBar");b.style.display="none";c.className="akeeba-progress-fill--failure";document.getElementById("extractErrorText").innerHTML=a;document.getElementById("extractError").style.display="block"};akeeba.Update.nextStep=function(){window.location=akeeba.Update.nextStepUrl};akeeba.Update.onGenericError=function(a){alert(a)};
akeeba.Update.setProgressBar=function(a,b){void 0==b&&(b="downloadProgressBar");a=1>=a?100*a:a;0>a&&(a=0);100<a&&(a=100);var c=document.getElementById(b),d=document.getElementById(b+"Info");c.className=100>a?"akeeba-progress-fill":"akeeba-progress-fill--success";document.getElementById(b).style.width=a.toFixed(1)+"%";d.innerText=a.toFixed(1)+"%";Piecon.setProgress(a)};
akeeba.Update.humanFileSize=function(a,b){var c=b?1E3:1024;if(a<c)return a+" B";b=b?"kB MB GB TB PB EB ZB YB".split(" "):"KiB MiB GiB TiB PiB EiB ZiB YiB".split(" ");var d=-1;do a/=c,++d;while(a>=c);return a.toFixed(1)+" "+b[d]};
...
akeeba.Update.stepDownload=function(a){if(a.status){var b=0,c=0,d=0,e=-1;void 0!=a.totalSize&&(b=a.totalSize);void 0!=a.doneSize&&(c=a.doneSize);void 0!=a.percent&&(d=a.percent);void 0!=a.frag&&(e=a.frag);akeeba.Update.setProgressBar(d,"downloadProgressBar");document.getElementById("downloadProgressBarText").innerText=d.toFixed(1)+"%";post={task:"downloader",json:JSON.stringify({frag:e,totalSize:b,doneSize:c})};if(100>d)akeeba.System.doAjax(post,function(a){akeeba.Update.stepDownload(a)},null,!1);
else{akeeba.Update.setProgressBar(100,"downloadProgressBar");try{Piecon.reset()}catch(f){}akeeba.Update.nextStep()}}else 300<=a.errorCode&&500>a.errorCode?akeeba.Update.downloadErrorCallback(akeeba.Update.translations.ERR_INVALIDDOWNLOADID):akeeba.Update.downloadErrorCallback(a.error)};
...
akeeba.Update.finalizeUpdate=function(){akeeba.System.doEncryptedAjax({task:"finalizeRestore",factory:akeeba.Update.factory},function(a){window.location=akeeba.Update.finaliseUrl})}; //# sourceMappingURL=update.min.map
