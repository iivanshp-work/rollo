...
akeeba.Restore.doAjax=function(a,b,c){json=JSON.stringify(a);a={json:json};0<akeeba.Restore.password.length&&(a.password=akeeba.Restore.password);var e=(new Date).getTime()/1E3,g=parseInt(e,10);a._cacheBustingJunk=Math.round(1E3*(e-g));akeeba.Ajax.ajax({type:"POST",url:akeeba.Restore.ajaxURL,cache:!1,data:a,timeout:6E5,success:function(a){var f=a.indexOf("###");if(-1==f)a="Invalid AJAX data: "+a,null==c?null!=akeeba.Restore.errorCallback?akeeba.Restore.errorCallback(a):akeeba.Restore.errorCallbackDefault(a):
c(a);else{0!=f&&(a.substr(0,f),a=a.substr(f));a=a.substr(3);f=a.lastIndexOf("###");a=a.substr(0,f);try{var h=JSON.parse(a)}catch(d){errorMessage=d.message+"\n<br/>\n<pre>\n"+a+"\n</pre>";null==c?null!=akeeba.Restore.errorCallback?akeeba.Restore.errorCallback(errorMessage):akeeba.Restore.errorCallbackDefault(errorMessage):c(errorMessage);return}b(h)}},error:function(a,b,e){e=a.responseText?a.responseText:"";var d="<strong>AJAX Loading Error</strong><br/>HTTP Status: "+a.status+" ("+a.statusText+")<br/>";
d=d+"Internal status: "+b+"<br/>XHR ReadyState: "+a.readyState+"<br/>";d=d+"Raw server response:<br/>"+akeeba.System.sanitizeErrorMessage(e);null==c?null!=akeeba.Restore.errorCallback?akeeba.Restore.errorCallback(d):akeeba.Restore.errorCallbackDefault(d):c(d)}})};
akeeba.Restore.startTimeoutBar=function(a,b){akeeba.Restore.resetTimeoutBar();akeeba.Restore.timer=setInterval(function(){akeeba.Restore.lastResponseSeconds++;var a=akeeba.Restore.translations.COM_AKEEBA_BACKUP_TEXT_LASTRESPONSE.replace("%s",akeeba.Restore.lastResponseSeconds.toFixed(0));try{document.getElementById("response-timer").querySelector("div.text").textContent=a}catch(e){}},1E3)};
