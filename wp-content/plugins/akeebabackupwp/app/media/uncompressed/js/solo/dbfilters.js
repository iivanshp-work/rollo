...
akeeba.Dbfilters.render = function (data)
{
	...
	for (var table in data.tables)
	{
		...
		for (var counter = 0; counter < available_filters.length; counter++)
		{
			...
			ui_icon.setAttribute('title', akeeba.Dbfilters.translations['COM_AKEEBA_DBFILTER_TYPE_' + filter.toUpperCase()]);
		...
		if (dbef.rows)
		{
			...
			uiTableSizeContainer.setAttribute('title', akeeba.Dbfilters.translations['COM_AKEEBA_DBFILTER_TABLE_META_ROWCOUNT']);
...
akeeba.Dbfilters.addRow = function (def, append_to_here)
{
	...
	var type_text = akeeba.Dbfilters.translations['COM_AKEEBA_DBFILTER_TYPE_' + def.type.toUpperCase()];
