...
<div style="display: none">
    <div id="akeeba-buadmin-{{ (int)$record['id'] }}" tabindex="-1" role="dialog">
        <div class="akeeba-renderer-fef {{ ($this->getContainer()->appConfig->get('darkmode', -1) == 1) ? 'akeeba-renderer-fef--dark' : '' }}">
            ...
            <p>
                <strong>@lang('COM_AKEEBA_BUADMIN_LBL_ARCHIVEPATH' . ($archiveExists ? '' : '_PAST'))</strong><br/>
                <span class="akeeba-label--information">
		        {{{ AkeebaHelperUtils::getRelativePath(APATH_BASE, dirname($record['absolute_path'])) }}}
		</span>
            </p>
            <p>
                <strong>@lang('COM_AKEEBA_BUADMIN_LBL_ARCHIVENAME' . ($archiveExists ? '' : '_PAST'))</strong><br/>
                <br/>
                <code>
		            {{{ $record['archivename'] }}}
                </code>
        ...
        <div class="akeeba-renderer-fef {{ ($this->getContainer()->appConfig->get('darkmode', -1) == 1) ? 'akeeba-renderer-fef--dark' : '' }}">
            ...
            @if ($record['multipart'] < 2)
                <a class="akeeba-btn--primary--small"
                   href="javascript:confirmDownload('{{ $thisID }}', '');">
            ...
            @if ($record['multipart'] >= 2)
                ...
                @for ($count = 0; $count < $record['multipart']; $count++)
                    ...
                    <a class="akeeba-btn--small--dark"
                       href="javascript:confirmDownload('{{ $thisID }}', '{{ urlencode($count) }}');">
...
<div style="padding-bottom: 3pt">
	@if ($showDownload)
        <a class="akeeba-btn--{{ $showManageRemote || $showUploadRemote ? 'smal--grey' : 'green' }}"
           href="javascript:akeeba.Manage.onDownloadClick('#akeeba-buadmin-download-{{ (int) $record['id'] }}')"
        >
            <span class="akion-android-download"></span>
			@lang('COM_AKEEBA_BUADMIN_LOG_DOWNLOAD')
        </a>
	...
	@if ($showViewLog)
        <a class="akeeba-btn--grey akeebaCommentPopover" {{ ($record['meta'] != 'obsolete') ? '' : 'disabled="disabled" onclick="return false;"' }}
    ...
    <a class="akeeba-btn--grey--small akeebaCommentPopover"
            href="javascript:akeeba.Manage.onShowInfoClick('#akeeba-buadmin-{{ (int)$record['id'] }}')"
