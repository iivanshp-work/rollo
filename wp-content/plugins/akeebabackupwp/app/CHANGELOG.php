...
Akeeba Solo 3.6.0
================================================================================
+ Option for automatic Dark Mode on Safari
...
# [HIGH] WordPress restoration: time limits are not respected when replacing data
# [HIGH] WordPress restoration: restoring to same URL but different filesystem path results in the absolute path to the site not being updated
# [HIGH] WordPress restoration fails when the connection information to the old site's database allows connection to a database but the database contains a WordPress installation on a different URL than the one backed up e.g. from http://www.example.com/foo to http://www.example.com/foo/bar (or even http://www.example.com/foo/foo)
# [HIGH] WordPress restoration fails when the connection information to the old site's database allows connection to a database but the database contains a WordPress installation on a different URL than the one backed up
# [HIGH] WordPress restoration doesn't work properly when the original site's URL and the restored site's URL have a similar but not identical string in the path and / or (sub)domain e.g. backing up http://localhost/ample and restoring to http://example.com
# [MEDIUM] Fixed Javascript error in Database Tables Exclusion page
...
# [HIGH] Getting the database driver list causes a crash if PDO is not enabled
...
- Removed platforms with minimal to no use: Drupal 7, Drupal 8, Grav, Magento 1.x, Magento 2.x, Moodle, OctoberCMS, PageKit, phpBB
~ WordPress data replacement is now faster and more reliable
~ Providing the URL to Akeeba Solo during setup is now optional (Akeeba Solo will work on most servers without it)
...
- Removed SQLite as supported database for installation
...
# [LOW] Fixed display issue while switching database driver in the Wizard page
...
Akeeba Solo 3.1.1
================================================================================
! Using wp-cli plugin update results in PHP Fatal Error when Akeeba Backup for WordPress is installed
...
# [HIGH] One time emergency passwords are not visible in the user edit page after enabling Two Factor Authentication
# [HIGH] ANGIE for Drupal: It wasn't possible to restore a Drupal website
# [MEDIUM] Drupal: Fixed reading site settings inside the Configuration Wizard
# [MEDIUM] Installer (ANGIE) language files not included in the backup
# [MEDIUM] Default backup file permissions should be 0644, not 0755# [LOW] Obsolete PHP version warning appears twice
